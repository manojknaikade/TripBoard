...
BEGIN
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Cannot cut over trips.vehicle_id to UUID while null vehicle_uuid rows remain.';
    END IF;
END
$$;
...
ALTER TABLE public.trips
    RENAME COLUMN vehicle_uuid TO vehicle_id;

ALTER TABLE public.trips
    ALTER COLUMN vehicle_id SET NOT NULL;

ALTER TABLE public.trips
    DROP CONSTRAINT IF EXISTS trips_vehicle_uuid_fkey;
...
DROP INDEX IF EXISTS public.idx_trips_vehicle_uuid_start_time;

CREATE INDEX IF NOT EXISTS idx_trips_vehicle_id_start_time
    ON public.trips USING btree (vehicle_id, start_time DESC);

DROP FUNCTION IF EXISTS public.get_trip_list_summary(timestamp with time zone, timestamp with time zone, uuid);
...
USING (
    EXISTS (
        ...
        WHERE public.vehicles.id = public.trips.vehicle_id
...
USING (
    EXISTS (
        ...
        WHERE vehicles.id = public.trips.vehicle_id
