CREATE UNIQUE INDEX IF NOT EXISTS idx_trip_waypoints_trip_id_timestamp
ON public.trip_waypoints(trip_id, timestamp);
...
CREATE OR REPLACE FUNCTION public.backfill_trip_waypoints()
RETURNS integer
LANGUAGE plpgsql
SET search_path = public
AS $$
...
SELECT public.backfill_trip_waypoints();

COMMENT ON FUNCTION public.backfill_trip_waypoints() IS
'Backfills exact trip routes from telemetry_raw into trip_waypoints for historical trips.';
