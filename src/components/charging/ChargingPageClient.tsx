...
import { fetchReverseGeocode, readCachedReverseGeocode } from '@/lib/client/geocode';
...
export type ChargingListResponse = {
    success?: boolean;
...
async function getChargingLocationLabel(session: ChargingSession): Promise<string> {
    ...
    try {
        const data: GeocodeResult = await fetchReverseGeocode(session.latitude, session.longitude);
...
type ChargingPageClientProps = {
    initialData?: ChargingListResponse | null;
};

export default function ChargingPageClient({
    initialData = null,
}: ChargingPageClientProps) {
    ...
    const [sessions, setSessions] = useState<ChargingSession[]>(() => Array.isArray(initialData?.sessions) ? initialData.sessions : []);
    const [summary, setSummary] = useState<ChargingSummary | null>(() => initialData?.summary || null);
    const [loading, setLoading] = useState(!initialData);
    const [loadingMore, setLoadingMore] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [nextOffset, setNextOffset] = useState<number | null>(() => (
        initialData
            ? (typeof initialData.nextOffset === 'number' ? initialData.nextOffset : null)
            : 0
    ));
    ...
    const fetchSessions = useCallback(async ({ reset, offset }: { reset: boolean; offset: number }) => {
        ...
        try {
            ...
            const data = await fetchCachedJson<ChargingListResponse>(
                requestCacheKey,
                async () => {
    ...
    useEffect(() => {
        ...
        if (!initialData?.success || timeframe !== '7days' || customStart || customEnd) {
            return;
        }

        const params = getChargingRequestParams(0, true);
        writeCachedJson(
            `charging:list:${params.toString()}`,
            initialData,
            CHARGING_BOOTSTRAP_CACHE_TTL_MS
        );
    }, [customEnd, customStart, getChargingRequestParams, hasCompleteDateRange, initialData, timeframe]);
...
function SessionCard({
    ...
    const [resolvedLocationLabel, setResolvedLocationLabel] = useState<string | null>(() => {
        ...
        const cached = readCachedReverseGeocode(session.latitude, session.longitude);
        return cached ? extractLocalityLabel(cached, session) : null;
    });
