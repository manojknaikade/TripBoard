...
const TRIP_EXACT_THUMBNAIL_CACHE_TTL_MS = 5 * 60_000;
...
type TripsPageClientProps = {
    initialData?: TripsListResponse | null;
};
...
export default function TripsPageClient({
    ...
    const [trips, setTrips] = useState<Trip[]>(() => Array.isArray(initialData?.trips) ? initialData.trips : []);
    const [summary, setSummary] = useState<TripsSummary | null>(() => initialData?.summary || null);
    ...
    const fetchTrips = useCallback(async ({ reset, offset }: { reset: boolean; offset: number }) => {
        ...
        try {
            ...
            const data = await fetchCachedJson<TripsListResponse>(
                requestCacheKey,
                async () => {
    ...
    useEffect(() => {
        ...
        const params = getTripsRequestParams(0, true);
        writeCachedJson(
            `trips:list:${params.toString()}`,
            initialData,
            TRIPS_BOOTSTRAP_CACHE_TTL_MS
        );

        return undefined;
    }, [customEnd, customStart, getTripsRequestParams, hasCompleteDateRange, initialData, timeframe]);
